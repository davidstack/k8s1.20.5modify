...
function kube::release::create_docker_images_for_server() {
  ...
  (
    ...
    # registry.cn-hangzhou.aliyuncs.com/google_containers is the constant tag in the docker archives, this is also the default for config scripts in GKE.
    # We can use KUBE_DOCKER_REGISTRY to include and extra registry in the docker archive.
    # If we use KUBE_DOCKER_REGISTRY="registry.cn-hangzhou.aliyuncs.com/google_containers", then the extra tag (same) is ignored, see release_docker_image_tag below.
